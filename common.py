...
class IOArgs:
    ...
    filepath_or_buffer: pandas._typing.FileOrBuffer
    encoding: str
    mode: str
    compression: pandas._typing.CompressionDict
...
class IOHandles:
    ...
    handle: pandas._typing.Buffer
    compression: pandas._typing.CompressionDict
    created_handles: List[pandas._typing.Buffer] = dataclasses.field(default_factory=list)
...
def _expand_user(filepath_or_buffer: pandas._typing.FileOrBuffer[AnyStr]) -> pandas._typing.FileOrBuffer[AnyStr]:
    """
...
def stringify_path(
    filepath_or_buffer: pandas._typing.FilePathOrBuffer[AnyStr],
    convert_file_like: bool = False,
) -> pandas._typing.FileOrBuffer[AnyStr]:
    """
    ...
    if not convert_file_like and is_file_like(filepath_or_buffer):
        ...
        return cast(pandas._typing.FileOrBuffer[AnyStr], filepath_or_buffer)
...
def is_fsspec_url(url: pandas._typing.FilePathOrBuffer) -> bool:
    """
...
def _get_filepath_or_buffer(
    filepath_or_buffer: pandas._typing.FilePathOrBuffer,
    encoding: str = "utf-8",
    compression: pandas._typing.CompressionOptions = None,
    mode: str = "r",
    storage_options: pandas._typing.StorageOptions = None,
) -> IOArgs:
    """
...
def get_compression_method(
    compression: pandas._typing.CompressionOptions,
) -> Tuple[Optional[str], pandas._typing.CompressionDict]:
    """
...
def infer_compression(
    filepath_or_buffer: pandas._typing.FilePathOrBuffer, compression: Optional[str]
) -> Optional[str]:
    """
...
def get_handle(
    path_or_buf: pandas._typing.FilePathOrBuffer,
    mode: str,
    ...
    handles: List[pandas._typing.Buffer]
...
class _BytesZipFile(zipfile.ZipFile, BytesIO):  # type: ignore[misc]
    ...
    def __init__(
        self,
        file: pandas._typing.FilePathOrBuffer,
        mode: str,
...
def _maybe_memory_map(
    handle: pandas._typing.FileOrBuffer,
    memory_map: bool,
    encoding: str,
    mode: str,
    errors: Optional[str],
) -> Tuple[pandas._typing.FileOrBuffer, bool, List[pandas._typing.Buffer]]:
...
def file_exists(filepath_or_buffer: pandas._typing.FilePathOrBuffer) -> bool:
...
def _is_binary_mode(handle: pandas._typing.FilePathOrBuffer, mode: str) -> bool:
